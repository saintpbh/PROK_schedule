...
# 총무 동정 데이터 인코딩 수정 스크립트
# 1802번 줄부터 끝까지를 올바른 UTF-8 데이터로 교체

$file = "history_data.js"
$content = Get-Content $file -Encoding UTF8 -Raw

# Find the boundary line 1801 (last line of 총회장 동정 data before 총무 동정)
# We need to keep everything up to and including line 1801
$lines = Get-Content $file -Encoding UTF8
$keepLines = $lines[0..1800]  # lines 1 to 1801 (0-indexed: 0..1800)
$keepText = ($keepLines -join "`r`n") + "`r`n"

# New secretary section with correct UTF-8
$secretaryData = @'
  {
    "id": "1681_166852",
    "board_id": "1681",
  ...
  },
  {
    ...
    "content": "<div class=\"detail-content\"><p>10월 25일(금) 오후 2시 총회총무 이훈삼 목사 취임 감사예식이 열렸습니다. 총무직이 막중함을 알기에 참석하여 축하드리고 격려하였습니다.</p></div>"
  },
  {
    "id": "1681_166826",
    "board_id": "1681",
    ...
    "content": "<div class=\"detail-content\"><p>지난 4월부터 '대한기독교서회'에 교단 파송 이사가 되었습니다. 대한기독교서회 이사회가 있어서 처음으로 참석하였습니다. 이번 이사회는 코로나 이후 처음으로 말레이시아에서 이사들의 연수회와 함께 열렸습니다.</p></div>"
  },
  {
    "id": "1681_166824",
    "board_id": "1681",
    ...
    "content": "<div class=\"detail-content\"><p>2024년 9월 24일부터 26일까지 제 109회 총회를 하나님의 은혜 가운데 잘 마쳤습니다. 총회 둘째 날, 오전 첫 시간, 신구임원 이취임식 중, 특별히 저에게 시간을 주셔서 모든 총대님들 앞에서 총무 이임 인사를 드릴 수 있었습니다. 우리 교단 총회를 섬기는 총무(總務)로서 지난 4년, 참으로 행복했습니다!</p><p class=\"each-img\"><img src=\"http://data.dimode.co.kr/UserData/prok/files/1681/166824/MTcyNzIxNDE4NTc1OC5qcGdkaW1vZGVfMzI5MTQ5NF9lbmMc.jpg\" style=\"max-width:100%\" /></p></div>"
  },
  {
    "id": "1681_166815",
    "board_id": "1681",
...
$finalContent = $keepText + $secretaryData
...
$newLines = (Get-Content $file -Encoding UTF8).Count
Write-Host "Done. Lines: $newLines"
Write-Host "Check line 1802:"
Get-Content $file -Encoding UTF8 | Select-Object -Skip 1801 -First 5
